...
const handler = async (req: Request): Promise<Response> => {
  const { messages, model, key, signal } = await req.json();
  ...
  try {
    const openAIStream = await OpenAIStream(model, messages, key, FUNCTIONS, function_call, max_tokens, temperature, stream, signal);
